...
for (StrandInd in OBorOT) {
  ...
  for (ChrInd in ChrNames) {
    ...
    write.table(DMRRanksCpGSite, paste(DirSave,StrandInd,"/CpGSiteRank/CpGSiteRank_",toString(ChrInd),".txt",sep = ""),col.names = TRUE, row.names = FALSE)
    write.table(SubClusterInfo[SubClusterInfo$HyperFlag == TRUE ,!(names(SubClusterInfo) %in% "HyperFlag")],paste(DirSave,StrandInd,"/DMRInfo/Hyper/DMRInfo_",ChrInd,".txt",sep = ""),
                col.names = TRUE, row.names = FALSE)
    write.table(SubClusterInfo[SubClusterInfo$HyperFlag == FALSE,!(names(SubClusterInfo) %in% "HyperFlag")],paste(DirSave,StrandInd,"/DMRInfo/Hypo/DMRInfo_" ,ChrInd,".txt",sep = ""),
                col.names = TRUE, row.names = FALSE)
  }
}
